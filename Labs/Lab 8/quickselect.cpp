...
size_t lomuto_partition(int array[], size_t left, size_t right) {
    ...
    // Implemented pseudocode provided in class
    int pivot = array[left];
    size_t s = left;
    
    for(size_t y = left + 1; y <= right ; y++){
		
        if(array[y] < pivot){
			s = s + 1;
			swap(array[s], array[y]);
		}
...
int quick_select(int array[], size_t left, size_t right, size_t k) {
    ...
    if (s == k-1) {
        ...
        if (s < k-1) {
            return quick_select(array, x, right, k);
        }
        else {
            return quick_select(array, left, x-2, k);
        }
    }
    return 0;
}
